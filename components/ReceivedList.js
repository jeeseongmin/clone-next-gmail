...
const ReceivedList = () => {
	...
	const receivedList = keys.filter(function (element, index) {
		return myThread[element].received.length > 0;
	});
	...
	return (
		<div class="flex-1 h-full flex flex-col">
			<div class="w-full h-10 px-2 flex-grow-0 flex-shrink-0 flex flex-row justify-start items-center border-b-2 border-gray-100">
				...
				/>
				{/* <div class="h-full w-10 flex justify-start items-center"></div> */}
				<div class="h-full w-10 ml-6 p-2 flex justify-center items-center">
			...
			{receivedList.map((element, index) => {
				return <Mail thread={element} type="receive" />;
			})}
